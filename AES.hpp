...
inline const unsigned char SBox[256] = {
...
inline const unsigned char InvSBox[256] = {
...
protected:
    ...
    std::vector<unsigned char*> KeyExpansion(const unsigned char* key) {
        ...
        for (int idx = 0; idx < 4 * (Nr + 1); idx++) {
            roundKeys.push_back(new unsigned char[4]);
        }
        while (i < Nk) {
            for (int j = 0; j < 4; ++j) {
        ...
        while (i < 4 * (Nr + 1)) {
            const auto temp = new unsigned char[4];
            ...
            const unsigned char* xored = XorWords(roundKeys[i - Nk], temp);
            memcpy(roundKeys[i], xored, 4);
            delete[] xored;
            delete[] temp;
            i++;
        }
        std::vector<unsigned char*> expandedKeys(Nr + 1);
        for (int j = 0; j <= Nr; ++j) {
            expandedKeys[j] = new unsigned char[16];
            for (int k = 0; k < 4; ++k) {
                for (int l = 0; l < 4; ++l) {
                    expandedKeys[j][l + k * 4] = roundKeys[j * 4 + k][l];
                }
            }
        }
        for (const auto* ptr : roundKeys) {
            delete[] ptr;
        }
        return expandedKeys;
    }
    ...
    unsigned char* AddPadding(const unsigned char* input, const int inputSize) {
        ...
        for (int i = 0; i < paddingSize; i++) {
            ...
            temp[inputSize + i] = static_cast<unsigned char>(gen() % 256);
        }
        return temp;
    }
